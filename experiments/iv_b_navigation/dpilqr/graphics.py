...
def set_bounds(xydata, ax=None, zoom=0.1):
    ...
    xmarg = np.ptp(xydata[:, 0]) * zoom
    ymarg = np.ptp(xydata[:, 1]) * zoom
    ax.set(
...
def _setup_gif(axes, X, xf, x_dims, radius, distances):
    ...
    for _, c in zip(range(n_agents), cycle(plt.cm.tab20.colors)):
        handles1.append(
            (
                # Increased markersize (e.g., to 15) and added linewidth (e.g., to 3)
                ax1.plot(0, c=c, marker="o", markersize=15, linewidth=3)[0],
                ax1.add_artist(
                    # Increased lw (line width) for the circle border (e.g., from 6 to 9)
                    plt.Circle(
                        (np.nan, np.nan), radius, color="k", fill=True, alpha=0.3, lw=9
                    )
                ),
            )
        )
    ...
    for xg in split_agents(xf, x_dims):
        # Added 's' to increase marker size, and 'linewidths' for the cross thickness
        ax1.scatter(xg[0, 0], xg[0, 1], c="r", marker="x", zorder=10, s=150, linewidths=3)

    X_cat = np.vstack(split_agents(X, x_dims))
    set_bounds(X_cat, axes[0], zoom=0.15)
    
    # Increased the title font size
    ax1.set_title("Trajectories", fontsize=18)
    
    # NEW: Increase the font size of the axis tick labels (the numbers on the axes)
