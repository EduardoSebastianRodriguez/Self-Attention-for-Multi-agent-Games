...
class Experiment(CallbackNotifier):
    ...
    def __init__(
        ...
        if freeze_config:
            ...
            """
            load_groups = freeze_config.get("load_groups", ["adversary"])
            freeze_group = freeze_config.get("freeze_group", "adversary")
            ...
            assert(model_path)
            self.load_group_network(model_path, load_groups, actor_critic, restore_map_location)

            # Determine the policy module index for the group
            groups = list(self.group_map.keys())
            group_index = groups.index(freeze_group)
            ...
            self.optimizers = {
                curr_group: {
                    ...
                    for loss_name, params in self.algorithm.get_parameters(curr_group).items()
                }
                for curr_group in self.group_map.keys() if curr_group != freeze_group
            }

            self.valid_groups.remove(freeze_group)
    ...
    def _perform_checks(self):
        ...
        if self.algorithm_config in (MappoConfig, ):
    ...
    @local_seed()
    @torch.no_grad()
    def _evaluation_loop(self, get_dist_metrics=False, plot_metrics=False, get_time_metrics=False):
        metrics = None
        ...
        with set_exploration_type(
            ...
            if self.test_env.batch_size == ():
                ...
                if get_dist_metrics:
                    avg_metrics, std_metrics, per_env_metrics = self.get_distance_metrics(rollouts)
                    print(f"Average mean min distance across episode and environments: {avg_metrics}")
                    print(f"Average standard deviation of min distances in all the episodes across envs: {std_metrics}")
                    if plot_metrics:
                        self.plot_dist_metrics(per_env_metrics)
        ...
        # I know we've been running vectorized evaluation instead of one-by-one, but I'd rather put metrics down here just in case
        if get_time_metrics:
            catch = self.check_collisions(rollouts)
            if plot_metrics:
                average_catches, std_catches = self.plot_cum_time(catch)
                print(f"Average number of catches: {average_catches}")
                print(f"Standard deviation of number of catches: {std_catches}")
                self.plot_catch_diff(catch)
        # Callback
        self._on_evaluation_end(rollouts)
        return metrics
    
    def get_distance_metrics(self, rollouts):
        sum_radii = 0.125
        rollouts = rollouts.permute(1, 0)
        num_steps, num_envs = rollouts.shape[0], rollouts.shape[1]
        min_distances = torch.zeros(num_steps, num_envs)

        # fig, ax = plt.subplots(figsize=(10, 10))

        # positions = []  ## This will be a 100 x 6 x 2 tensor
        # for step in rollouts:
        #     adv_pos_env1 = step['adversary']['observation']['pos'][0]
        #     agent_pos_env1 = step['agent']['observation']['pos'][0]
        #     agent_pos_list = []
        #     for adv_pos in adv_pos_env1:
        #         agent_pos_list.append(list(adv_pos))
        #     for agent_pos in agent_pos_env1:
        #         agent_pos_list.append(list(agent_pos))
        #     positions.append(agent_pos_list)

        # positions = torch.tensor(positions).permute(1, 0, 2)
        # colors = ['red', 'blue', 'green', 'orange', 'cyan', 'pink']
        # for agent, color in zip(positions, colors):
        #     for step in range(1, positions.shape[1]):
        #         prev_pos = agent[step - 1]
        #         curr_pos = agent[step]
        #         plt.axline(tuple(prev_pos), tuple(curr_pos), color=color)
        # plt.show()

        for index, step in enumerate(rollouts):
            adversary_pos = step['adversary']['observation']['pos']  # [B, num_adv, 2]
            agent_pos = step['agent']['observation']['pos']  # [B, num_agents, 2]
            
            # Compute pairwise differences
            diff = adversary_pos.unsqueeze(2) - agent_pos.unsqueeze(1)  # [B, num_adv, num_agents, 2]
            
            # Calculate Euclidean distances and adjust for agent radii
            raw_dists = torch.norm(diff, dim=-1) - sum_radii  # [B, num_adv, num_agents]
            
            # Ensure non-negativity using ReLU (clamps negative values to 0)
            non_negative_dists = torch.relu(raw_dists)  # [B, num_adv, num_agents]
            
            # Find minimum distance per environment
            min_dists = non_negative_dists.view(num_envs, -1).min(dim=-1)[0]  # [B]
            min_distances[index] = min_dists

        min_distances = min_distances.permute(1, 0)
        avg_across_steps = min_distances.mean(dim=-1)
        std_across_steps = min_distances.std(dim=-1)
        avg_across_envs = avg_across_steps.mean()
        avg_std_across_envs = std_across_steps.mean()     

        return avg_across_envs, avg_std_across_envs, min_distances
    
    def check_collisions(self, rollouts):
        '''
            counts the number of agents caught (colliding with adversary) at every evaluation step for each rollout
            count is cumulative over time (monotonic increasing function)
            only checks agent collisions in case multiple adversaries are touching the same agent
        ...
        for step in rollouts:
            # adversary_collisions = step['adversary']['info']['catch']   # [self.max_steps, num_adv, 1]
            agent_collisions = step['agent']['info']['catch']           # [self.max_steps, num_age, 1]
            age_coll = torch.squeeze(agent_collisions)          # [self.max_steps, num_age]
            num_catch = torch.count_nonzero(age_coll, dim=-1)   # [self.max_steps]
            catch.append(num_catch)
        
        return catch    # [self.config.evaluation_episodes, self.max_steps]
    ...
    def load_actor_networks(self, state_dict: Dict):
        ...
        adversary_actor_network.pop("__batch_size", None)
        adversary_actor_network.pop("__device", None)
        
        # Filter out exploration parameters
        exploration_keys = ["sigma_init", "sigma_end", "mean", "std", "sigma"]
        adversary_actor_network = {k: v for k, v in adversary_actor_network.items() 
                                if not any(exp_key in k for exp_key in exploration_keys)}
        
        self.policy[0].load_state_dict(adversary_actor_network, strict=False)
        ...
        agent_actor_network.pop("__batch_size", None)
        agent_actor_network.pop("__device", None)
        
        # Filter out exploration parameters
        agent_actor_network = {k: v for k, v in agent_actor_network.items() 
                            if not any(exp_key in k for exp_key in exploration_keys)}
        
        self.policy[1].load_state_dict(agent_actor_network, strict=False)
    ...
    def _load_experiment(self) -> Experiment:
        ...
        self.load_actor_networks(loaded_dict)
        return self
    
    def load_group_network(self, model_path, groups, actor_critic, restore_map_location):
    ...
    def plot_dist_metrics(self, metrics):
        # metrics = torch.stack(metrics).T
        num_envs = metrics.shape[0]
        num_steps = metrics.shape[1]

        # Setup plot
        fig, ax = plt.subplots(figsize=(12, 6))
        colors = cm.viridis(np.linspace(0, 1, num_envs))  # Colormap

        # Plot each environment's metrics
        for i, (env_metrics, color) in enumerate(zip(metrics, colors)):
            ax.plot(range(num_steps), env_metrics, label=f'Env #{i}', color=color)

        # Beautify plot
        ax.set_title("Metrics per Environment Over Time", fontsize=16)
    ...
    def plot_cum_time(self, catch):
        '''
            want to plot total (cumulative) catch over time
                - all environments in evaluation batch; [self.config.evaluation_episodes] # of lines
                - average and stddev over all evaluation episodes (at each step)

            IN:
                catch [list]: [self.config.evaluation_episodes][self.max_steps]
        '''
        plots_folder = self.folder_name / "plots"
        plots_folder.mkdir(parents=False, exist_ok=True)

        eval_range = np.arange(self.max_steps)      # steps in the evaluation rollout (X-axis)
        all_total_caught = np.cumsum(np.asarray(catch), axis=1)     # cumulative tally of each time an agent is caught, for each episode

        fig, ax = plt.subplots()
        for episode in range(self.config.evaluation_episodes):
            ax.plot(eval_range, all_total_caught[episode], label=f"Episode {episode}")

        ax.set_title(f"Cumulative number of agents caught over time ({self.config.evaluation_episodes} episodes)", fontsize=16)
        ax.set_ylabel("Cumulative number of times any agent is caught")
        ax.set_xlabel("Evaluation timestep")
        plt.savefig(plots_folder / f"all_cum_caught_{self.n_iters_performed}.png", bbox_inches="tight")

        # Plotting average with stddev bars
        step_interval = 1  # interval of steps between average
        avg_range = np.arange(0, self.max_steps, step_interval)
        avg_cum_caught = np.average(all_total_caught, axis=0)     # average (over all episodes) at each evaluation step
        std_total_caught = np.std(all_total_caught, axis=0)     # stddev to go along with average
        fig, ax = plt.subplots()
        ax.errorbar(avg_range, avg_cum_caught[avg_range], std_total_caught[avg_range], marker="o")
        ax.set_title("Average of cumulative agents caught versus time", fontsize=16)
        ax.set_ylabel("Average number of times an agent was caught")
        ax.set_xlabel("Evaluation timestep")
        plt.savefig(plots_folder / f"avg_caught_{self.n_iters_performed}.png", bbox_inches="tight")
        for idx in range(self.max_steps):
            self.logger.log({"metric/time_avg_catch": avg_cum_caught[idx]}, step=idx)
            for jdx in range(self.config.evaluation_episodes):
                self.logger.log({"metric/time_total_catch": all_total_caught[jdx][idx]}, step=idx)
        
        return all_total_caught[:, -1].mean(), all_total_caught[:, -1].std()

    def plot_catch_diff(self, catch):
        '''
            plot differences in time between an agent being caught
                - all environments in evaluation batch; [self.config.evaluation_episodes] # of lines
                - average and stddev over all evaluation episodes (at each step)
        ...
        avg_caught = np.average(np.asarray(catch), axis=0)
        fig, ax = plt.subplots()
        ax.plot(eval_range, avg_caught)

        ax.set_title("Average agents caught at time", fontsize=16)
        ax.set_ylabel(f"Average agents caught (over {self.config.evaluation_episodes} episodes)")
        ax.set_xlabel("Evaluation timestep")
        plt.savefig(plots_folder / f"catch_diff_{self.n_iters_performed}.png", bbox_inches="tight")
